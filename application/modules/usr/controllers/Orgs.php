...
class Orgs extends MY_Admin
{
	...
	function __init(){
		...
		if(!($this->db->table_exists('orgs'))){
			...
			$attr = array('ENGINE' => 'MyISAM');
			
			$this->dbforge->create_table('orgs',FALSE,$attr);
		}
	}
	*/
	...
	function index()
	{
		$mydata['auth_meta'] = $this->meta('usr/orgs/', true);
	...
	function __search($keywords = '')
	{
		...
		foreach ($fields as $k => $v) {
			//$this->db->or_like('upper('.$this->data['table_name'].'.'.$v.')',$keywords);
