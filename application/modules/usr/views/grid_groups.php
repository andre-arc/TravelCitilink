	...
	<div class="row">
		<div class="col-md-12">
			<div class="panel">
				...
				<table id="grid_kec" data-show-refresh="true" data-classes="table table-no-bordered" data-pagination="true" data-id-field="id" data-page-list="[10, 25, 50, 100, ALL]" data-side-pagination="server">
				</table>
...
<script>
	...
	$(document).ready(function() {
		...
		$('#grid_kec').bootstrapTable({
			...
			search: true,
			pageSize: 10,
			url: SITE_URL + '/usr/groups/get_json/',
			singleSelect: true,
			columns: [{
					field: 'state',
					checkbox: true,
		...
		<?php if (($auth_meta['add']) || ($auth_meta['edit'])) : ?>
			$('#frm-wil-gp').submit(function(e) {
				...
				var url_form = ($('#act').val() == 'edit') ? SITE_URL + "/usr/groups/act_edit/" : SITE_URL + "/usr/groups/act_add/";
				$.ajax({
					type: "POST",
					url: url_form,
					dataType: "json",
			...
			$('#btn-del').click(function(e) {
				...
				if (rowSel.length) {
					...
					if (r == true) {
						$.ajax({
							type: "POST",
							url: SITE_URL + "/usr/groups/act_del/",
							dataType: "json",
							data: {
								id: rowSel[0].id
							},
