			...
			<div class="panel">
				...
				<div id="toolbar">
					<?php if ($auth_meta['add']) : ?>
						<a id="btn-add" class="btn btn-primary btn-sm" href="<?php echo site_url('usr/groups_menu/add/'); ?>" alt="ADD">
							<i class="fa fa-plus-circle"></i>&nbsp;Add
						</a>
					<?php endif; ?>
					<?php if ($auth_meta['edit']) : ?>
						<a id="btn-edit" class="btn btn-info btn-sm" href="<?php echo site_url('usr/groups_menu/edit/'); ?>" alt="Edit">
							<i class="fa fa-pencil"></i>&nbsp;Edit
						</a>
					<?php endif; ?>
					<?php if ($auth_meta['del']) : ?>
						<a id="btn-del" class="btn btn-danger btn-sm" href="<?php echo site_url('usr/groups_menu/del/'); ?>" alt="Del">
...
<script>
	...
	$(document).ready(function() {
		...
		$('#grid_kec').bootstrapTable({
			...
			search: true,
			pageSize: 10,
			url: SITE_URL + '/usr/groups_menu/get_json/',
			pageSize: 10,
			singleSelect: true,
			columns: [{
					field: 'state',
		...
		<?php if (($auth_meta['add']) || ($auth_meta['edit'])) : ?>
			...
			$('#frm-wil-gp').submit(function(e) {
				...
				var url_form = ($('#act').val() == 'edit') ? SITE_URL + "/usr/groups_menu/act_edit/" : SITE_URL + "/usr/groups_menu/act_add/";
				$.ajax({
					type: "POST",
					url: url_form,
					dataType: "json",
			...
			$('#btn-del').click(function(e) {
				...
				if (rowSel.length) {
					...
					if (r == true) {
						$.ajax({
							type: "POST",
							url: SITE_URL + "/usr/groups_menu/act_del/",
							dataType: "json",
							data: {
								id: rowSel[0].id
							},
