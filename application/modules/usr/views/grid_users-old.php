			...
			<div class="panel">
				...
				<div id="toolbar">
					<?php if ($auth_meta['add']) : ?>
						<a id="btn-add" class="btn btn-primary btn-sm" href="<?php echo site_url('usr/users/add/'); ?>" alt="ADD">
							<i class="fa fa-plus-circle"></i>&nbsp;Add
						</a>
					<?php endif; ?>
					<?php if ($auth_meta['edit']) : ?>
						<a id="btn-edit" class="btn btn-info btn-sm" href="<?php echo site_url('usr/users/edit/'); ?>" alt="Edit">
							<i class="fa fa-pencil"></i>&nbsp;Edit
						</a>
					<?php endif; ?>
					<?php if ($auth_meta['del']) : ?>
						<a id="btn-del" class="btn btn-danger btn-sm" href="<?php echo site_url('usr/users/del/'); ?>" alt="Del">
							<i class="fa fa-trash-o"></i>&nbsp;Del
						</a>
					<?php endif; ?>
				</div>
				<table id="grid_users" data-show-refresh="true" data-show-export="true" data-classes="table table-no-bordered" data-pagination="true" data-id-field="id" data-page-list="[10, 25, 50, 100, ALL]" data-side-pagination="server">
				</table>
...
<script>
	...
	$(document).ready(function() {
		...
		$('#grid_users').bootstrapTable({
			...
			search: true,
			pageSize: 10,
			url: SITE_URL + '/usr/users/get_json/',
			pageSize: 10,
			singleSelect: true,
			columns: [{
					field: 'state',
		...
		<?php if (($auth_meta['add']) || ($auth_meta['edit'])) : ?>
			$('#frm-adm-users').submit(function(e) {
				...
				var url_form = ($('#act').val() == 'edit') ? SITE_URL + "/usr/users/act_edit/" : SITE_URL + "/usr/users/act_add/";
			...
			$('#btn-del').click(function(e) {
				...
				if (rowSel.length) {
					...
					if (r == true) {
						$.ajax({
							type: "POST",
							url: SITE_URL + "/usr/users/act_del/",
							dataType: "json",
							data: {
								id: rowSel[0].id
							},
