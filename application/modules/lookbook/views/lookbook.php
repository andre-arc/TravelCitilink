					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" id="no_kk" name="no_kk" class="form-control input-sm" value="" />
								<small class="form-text text-muted" id="cek-nama"></small>
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
								<label class="control-label">Jenis/Unit</label>
								<select name="jenis_unit" class="form-control select" style="width:100%;" id="jenis_unit" required>
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" id="ket_barang" name="ket_barang" class="form-control input-sm" value="" placeholder="ex. Barang Gudang" />
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" id="stok" name="stok" class="form-control input-sm" value="" />
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
								<label class="control-label">Satuan</label>
								<input type="text" id="satuan_barang" name="satuan_barang" class="form-control input-sm" value="" />
							</div>
						</div>
					</div>
				</div>
...
<script>
	...
	$(document).ready(function(){
		...
		$('#grid_kec').bootstrapTable({
			...
			search:true,
			pageSize:10,
			url: SITE_URL+'/lookbook/get_json/',
			singleSelect:true,
			columns: [
			{
				field: 'state',
		...
		$('#btn-edit').click(function(e){
			...
			if(rowSel.length){
				...
				//load row
				$('#id').val(rowSel[0].id_barang);
				$('#nm_barang').val(rowSel[0].nm_barang);
				$('#jenis_unit').val(rowSel[0].jenis_unit).trigger('change');
				$('#merk').val(rowSel[0].merk);
				$('#stok').val(rowSel[0].stok);
				$('#ket_barang').val(rowSel[0].ket_barang);
				$('#satuan_barang').val(rowSel[0].satuan_barang);
		...
		<?php if(($auth_meta['add'])||($auth_meta['edit'])):?>
			$('#frm-wil-gp').submit(function(e){
				...
				var url_form = ($('#act').val()=='edit') ? SITE_URL+"/barang/act_edit/" : SITE_URL+"/barang/act_add/";
				$.ajax({
					type: "POST",
					url: url_form,
					dataType: "json",
			...
			if(rowSel.length){
				swal({
					...
					$.ajax({
						...
						dataType: "json",
						data: {id_pembeli:rowSel[0].id_pembeli},
						success: function(data){
							if(data.success){
								swal('Selamat', data.msg, 'success');
								$('#grid_kec').bootstrapTable('refresh');
